...
two light knobs, one for forward the other for reverse, round out the main
controls.  In addition, a screen and several buttons are available to
navigate the various configuration menus and toggle functions on or off
during operation.

![](img/mrbw-cst-annotated.jpg)

<a id="no-magic"></a>One key piece of information to remember about the
ProtoThrottle...  there is ***No Magic***.  The ProtoThrottle sends standard
DCC speed, direction, and function commands to your decoder, via the command
station.  Sometimes there may be a little sleight-of-hand behind the scenes
to fool your brain, but fundamentally, under the hood, the ProtoThrottle is
doing nothing different than your standard DCC throttle.
...
* **Be methodical.  Take things step-by-step.**  It may seem slow at first, or
  even cumbersome - "I just want to operate!".  But don't try to do
  everything at once.  Especially don't jump to the more advanced features
  first.  Take your time, go step-by-step and follow the instructions
  outlined below.  After completing your first setup, it *will* go faster
  and you can take some short cuts.  But please - don't do that the first
  time.
...
* **Test, Test... and test some more!**  This cannot be stressed enough. 
  ...
  changes, then try things, and be left wondering why it doesn't work.  Test
  each step of the way.  That way, if something doesn't work as you
...
itself.  A description of these tools are available over [here]() and you are
...
and fun (i.e.  the "90%") just by doing these three things.  That's the
heart of the recipe.
...
After configuring those three basic things, you will be able to enjoy
operating your locomotive with the ProtoThrottle.  Many people stop there
and are perfectly happy.  In fact, after going through the setup for those -
which is discussed below - we encourage you to stop.  Operate for a while. 
Take some mental notes about what you like, what you don't like.  What else
would you like the locomotive to do?  Spend some time
enjoying the ProtoThrottle experience and most of all... **have fun!**
...
specific brand of decoder.  Is that you cup of tea?  Great!  If not, no
...
addresses will be preceded by 's' on the screen whereas long addresses have
four numeric digits.
...
to dig deeper into why even your normal DCC system's throttle can't
communicate with the locomotive.  Maybe an electrical problem?  Maybe the
locomotive horn function is, in fact, not F2?  Solve that problem first
before proceeding.
...
take longer to get up to the requested speed and simulate more mass.

### Deceleration Momentum

When decreasing speed, the deceleration momentum setting is used instead. 
It is set by **CV 4**.  A higher value here means it will take longer to slow
down and to stop.  This can also be thought of as a coasting effect,
allowing the locomotive to keep moving (coast) even though you have
throttled down and are no longer "applying power" to the wheels.

### Operation Without Momentum

The following video shows operation with no momentum (CV 3 = 0, CV 4 = 0). 
Note the quick acceleration and stopping, which is not very prototypical.

**FIXME: insert video here**

### Momentum Recipe

**Acceleration:**  Start in the middle (CV 3 = 128).  Test it out.  Adjust
...
**Deceleration:**  Set to the max (CV 4 = 255).  This will provide maximum
...
Test and adjust CV 3 until you think you're happy.  You can always come back
...
This video shows operation with momentum.  In the first case, we have CV 3 =
160, which has been adjusted a little higher than the starting
recommendation above.  In the second case, we set CV 3 = 80.  Notice the
difference in the locomotive's response to notching up the throttle.

In both cases, the locomotive coasts nicely.  As mentioned above, since we
don't have the brake set up just yet, we have to rely on the emergency stop
...
## Brakes ![](img/braking.jpg){align=right style="max-width:50%"}

Why do we want brakes?  Brakes are a complement to the high momentum we set
in the previous section, in particular the high deceleration momentum.  This
counteracts the coasting effect from CV 4 being set to the maximum value. 
With braking, we now have a more prototypical operation of the throttle:
make the locomotive go with the throttle lever; make the locomotive stop with
brake lever.
...
There are variable braking effect options available, but we're going to
ignore those for now - if you don't have the basic on/off braking working,
trying to get something more advanced configured is a fool's errand
(remember that thing about taking it step-by-step?).
...
On the Tsunami2 decoder, the brake rates set above work in conjunction with
...
One final note.  Make sure CV 1.403 is set to zero.  If it is not, the brake
won't work as expected.  Many Athearn locomotives come from the factory with
CV 1.403 programmed to a non-zero value so you need to make sure to set this
...
The following video shows operation of the brake with a Tsunami2 decoder
configured as above.  It starts with the default settings of CV 4 = 255 and
CV 117 = CV 188 = 255.  Then, we adjust CV 4 to a value of 160 to increase
the braking.  Finally, CV 4 is adjusted down again to 128 for an even
...
Most decoders, especially factory installed ones, come with headlights that
automatically change directions when the locomotive direction changes. 
...
Therefore, the front headlight, read headlight, and front/rear ditch lights
(if equipped) should be assigned separate functions.

Since each decoder type configures the lighting outputs in different ways,
using different CVs and having different spare/available functions, and
everyone's preferences for functions to use or avoid differs, it is
difficult to give one specific recipe for light configuration.  We will
...
1. Assign front / rear headlights to different functions in decoder
    * Front = F0
    * Rear = F28 (or something unused)

1. Assign front / rear ditch lights to different functions in decoder
    * Front = F27 (or something unused)
    * Rear = F26 (or something unused)

1. Identify dim function
    * Tsunami2 = F7
    * Loksound 5 = F12
    * Check locomotive / decoders docs

4. Using the CONFIG FUNC menu on the ProtoThrottle, set the function
   numbers based on the example values above:
...
Now is the time to stop.  Operate your locomotive for a while.  Feel free to
go back and adjust things.  But most of all, have fun!

After spending some time with the ProtoThrottle, now you can start to
consider what advanced topics you might (or might not) want to tackle. 
These could include:
...
* Brake test
* Consisting… consist functions vs. individual

## Conclusion

Hopefully this tutorial has has given you the tools to confidently approach
setting up a locomotive for the ProtoThrottle.  Many thanks to the 
[STL RPM](https://stlrpm.com/) crew for allowing us to present this as a clinic
in 2025, to Scott Thornton and James McNab for help in preparing and
reviewing this material, and most of all, our ProtoThrottle customers for
your support!
