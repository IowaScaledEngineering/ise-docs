...
would like a refresher course, then [ProtoThrottle 101](pt101.md) is
...
itself.  A description of these tools are available [here](debug.md) and you are
...
Now, test it.  Pull the horn handle.  Assuming you have a factory configured
...
Before we get to setting up the momentum, let's take a look at how a
locomotive behaves with the typical default settings.  First, we accelerate
with CV 3 = 0.  Notice how the locomotive goes from stopped to moving almost
instantly.

![type:video](https://www.youtube.com/embed/SnItX9LbBDQ?si=7POS8HyF_18SFYYw)

Next, we stop the locomotive with a setting of CV 4 = 0.  Similar to above,
the locomotive comes to a stop almost instantly when the throttle handle is
moved to idle.

![type:video](https://www.youtube.com/embed/nURIJ1yzfaI?si=ledxLDIryc01GXAb)

Now, let's adjust the momentum to get more realistic behavior.

!!! warning "Stopping with High Momentum"
    Since we don't have the brake set up yet, we have to rely on the
    emergency stop (brake lever all the way to the right) to stop the
    locomotive.  You can turn this off later, but are advised to keep the
    emergency stop enabled on the ProtoThrottle brake handle until the brake
    is set up and working well in the next section.
...
The videos below show operation with momentum.  Notice how the locomotive
starts gradually and takes some time to reach the target speed.

![type:video](https://www.youtube.com/embed/VLZZtNqK0Zc?si=uxKV7OmsJwIFEhIs)

When throttling down, instead of stopping immediately the locmotive coasts
for an extended time.  This is exactly the behavior we want so that the
brake (coming up in the next section) can be used to bring the train to a
stop exactly where you want.

![type:video](https://www.youtube.com/embed/uey120KeFdo?si=FoXBDNt7bOa1BRS9)
...
!!! note "JMRI Decoder Pro"
    The values for CV 117 and CV 118 shown in the recipe above are the
    values to use if directly programming the CVs.  If using JMRI Decoder
    Pro, the Independent Brake (CV 117) and Train Brake (CV 118) fields are
    split into a Brake Rate and a Brake Rate Sign:

    ![](img/decoderpro-brake.png)

    Set the Brake Rate Sign to the "Subtract value..." setting.  Then, the
    Brake Rate is a value from 0 to 127, corresponding to the 128 to 255
    range shown above for CV 117 and CV 118.
...
The following video shows a light braking effect.  When the brake is
applied, the locomotive stops faster than when just coasting but still takes
some time to stop.

![type:video](https://www.youtube.com/embed/ronE7tAITts?si=Rcbp7iQx6XZDqPX6)

However, if you want a stronger braking effect, you can adjust the CVs as
explained above.  Here, we stop with a heavier braking effect programmed into
the decoder.

![type:video](https://www.youtube.com/embed/lURXASsZPQA?si=h3F1AOvw2yrVJyqe)

The key takeaway here is that you can (and should) experiment to get the
effect that feels right for you and your layout.
